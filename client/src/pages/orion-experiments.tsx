    ...
    try {
      ...
      if (source === "live") {
        await trainLiveMut.mutateAsync({
          algorithm,
          name: mName,
          hyperparameters: showHyperparams ? { n_estimators: nEstimators, max_depth: maxDepth, learning_rate: learningRate / 100 } : null,
        });
      } else {
                ...
                {showHyperparams && (
                  <div className="mt-3 space-y-3 border rounded-lg p-3 bg-muted/20">
                    <div>
                      ...
                      <Slider value={[nEstimators]} onValueChange={([v]) => setNEstimators(v)} min={50} max={500} step={50} data-testid="slider-n-estimators" />
                    </div>
                    <div>
                      <div className="flex justify-between text-xs mb-1"><span>max_depth</span><span className="font-mono">{maxDepth}</span></div>
                      <Slider value={[maxDepth]} onValueChange={([v]) => setMaxDepth(v)} min={2} max={20} step={1} data-testid="slider-max-depth" />
                    </div>
                    <div>
                      ...
                      <Slider value={[learningRate]} onValueChange={([v]) => setLearningRate(v)} min={1} max={50} step={1} data-testid="slider-learning-rate" />
                    </div>
                  </div>
                )}
              </div>
