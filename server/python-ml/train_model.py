...
def engineer_notebook_features(df):
...
def score_latest_active_customers(best_model, df_full, feature_cols, snapshot_col='snapshot_month'):
    ...
    try:
        ...
        sys.stderr.write("[score_latest_active] shap not installed; skipping latest-month scoring\n")
        return []
    ...
    try:
        ...
        # ---------------------------------------------------------------
        # SCORE-BASED RISK BANDS + HYBRID GROQ/RULE RECOMMENDATIONS
        # Uses fixed churn-probability thresholds for customer segmentation.
        # ---------------------------------------------------------------
        ...
        # Key from notebook Step 23 cell; env var takes precedence if set
        _GROQ_KEY_NOTEBOOK = "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"
        groq_client = None
        groq_api_key = os.environ.get('GROQ_API_KEY', '') or _GROQ_KEY_NOTEBOOK
        if groq_api_key:
            try:
...
""".strip()
        ...
        for idx, r in enumerate(results):
            risk_band = classify_risk_band_from_probability(r.get('churn_probability', 0.0))
...
def train_single_model(algorithm, X, y, feature_cols, custom_params=None, df=None,
    ...
    if model_family is None:
        raise ValueError(
            ...
            "Supported algorithms: Random Forest, LightGBM, XGBoost, Auto"
        )
...
def train_auto_unified(X, y, feature_cols, df=None, has_snapshot_month=False,
    ...
    coarse_combined_grid = [
        get_auto_param_grid('rf', stage='coarse'),
        get_auto_param_grid('lightgbm', stage='coarse'),
        get_auto_param_grid('xgboost', stage='coarse'),
    ]
    ...
    coarse_best_family = coarse_search.best_params_.get('model_family', 'lightgbm')
...
def main():
    ...
    try:
        ...
        supported_algorithms = {"Auto", "Random Forest", "LightGBM", "XGBoost"}
        if algorithm not in supported_algorithms:
            raise ValueError(
                f"Algorithm '{algorithm}' is not supported. "
                "Supported algorithms: Auto, Random Forest, LightGBM, XGBoost"
            )
