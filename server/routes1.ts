...
export async function registerRoutes(
  ...
  app.post("/api/models/train", async (req, res) => {
    try {
      ...
      // Use Python training for all algorithms
      // Unified Python training for EXPERT MODE
      if (algorithm) {
        ...
        const model = await storage.createMlModel({
          ...
          datasetId,
          algorithm: algorithm === "Auto" ? `Auto (${result.bestModel})` : algorithm,
          status: "trained",
  ...
  app.post("/api/models/train-live", async (req, res) => {
    try {
      ...
      // Use Python training for all algorithms
      // Unified Python live training for EXPERT MODE
      if (algorithm) {
        const result = await executePythonScript(
          "train_model.py",
          {
  ...
  app.get("/api/monitoring/:modelId", async (req, res) => {
    try {
      ...
      for (const prediction of modelPredictions) {
        ...
        if (riskCategory === "very high" || riskCategory === "high") highRiskCount += 1;
      ...
      const latestRealSnapshot: MonitoringSnapshot | null = totalPreds > 0
        ? {
          modelId,
          ...
          accuracy: baseAcc,
          recall: baseRecall,
          predictionVolume: totalPreds,
          highRiskPct: currentHighRiskPct,
          medRiskPct: currentMedRiskPct,
          lowRiskPct: currentLowRiskPct,
          psi: parseFloat(Math.min(0.35, Math.abs(baseAuc - baseAcc) * 0.6 + Math.max(0, currentHighRiskPct - 20) * 0.004).toFixed(4)),
          ks: parseFloat(Math.min(0.25, Math.abs(baseRecall - baseAcc) * 0.5 + Math.max(0, currentHighRiskPct - 15) * 0.0025).toFixed(4)),
          featureDriftScore: parseFloat(Math.min(0.3, Math.abs(baseAuc - baseRecall) * 0.5 + Math.max(0, currentHighRiskPct - 18) * 0.003).toFixed(4)),
      ...
      const snaps: MonitoringSnapshot[] = latestRealSnapshot
        ? [...syntheticBaseline, latestRealSnapshot]
        : syntheticBaseline;
      const realCount = latestRealSnapshot ? 1 : 0;

      // Build 12-point weekly chart data (most recent 12 snaps, or pad if fewer)
      const latest12 = snaps.slice(-12);
      const weeklyMetrics = latest12.map((snapshot: MonitoringSnapshot, index: number) => ({
        date: snapshot.date,
        label: `W${index + 1}`,
      ...
      // ── Feature importance over 6 periods (derived from model + time) ──
      ...
      const featureHistory: PeriodEntry[] = Array.from({ length: 6 }, (_, periodIdx) => {
      ...
      const driverChanges = features.map((f: any) => {
      ...
      const latestPsi = latestSnap?.psi ?? 0;
      const latestKs = latestSnap?.ks ?? 0;
      const latestHighRiskPct = latestSnap?.highRiskPct ?? 0;
