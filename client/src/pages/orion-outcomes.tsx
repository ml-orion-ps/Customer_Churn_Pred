                  ...
                  {(allPreds as any[]).map((p: any) => {
                    ...
                    return (
                      <tr key={p.id} className="border-t hover:bg-muted/20">
                        ...
                        <td className="p-3 text-muted-foreground max-w-[260px] truncate">{p.recommendedAction || "\u2014"}</td>
                      </tr>
                    );
                  })}
                  {(allPreds as any[]).length === 0 && <tr><td colSpan={4} className="p-6 text-center text-muted-foreground">No predictions yet. Train a model to generate predictions.</td></tr>}
                </tbody>
              </table>
            </div>
          </div>
