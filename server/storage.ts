...
import {
  customers, churnEvents, datasets, mlModels, predictions, recommendations, auditLog, modelEvaluationRuns,
  ...
  type InsertModelEvaluationRun, type ModelEvaluationRun,
} from "@shared/schema";
...
export interface IStorage {
  ...
  createModelEvaluationRun(data: InsertModelEvaluationRun): Promise<ModelEvaluationRun>;
  getModelEvaluationRuns(modelId: number): Promise<ModelEvaluationRun[]>;
}
...
export class DatabaseStorage implements IStorage {
  ...
  async getPredictions(modelId?: number): Promise<Prediction[]> {
    if (modelId) {
      return db
        ...
        .orderBy(desc(predictions.churnProbability), desc(predictions.predictedAt));
    }
    return db
      .select({
        id: predictions.id,
  ...
  async createModelEvaluationRun(data: InsertModelEvaluationRun): Promise<ModelEvaluationRun> {
    const [run] = await db
      .insert(modelEvaluationRuns)
      .values(data)
      .onConflictDoUpdate({
        target: [modelEvaluationRuns.modelId, modelEvaluationRuns.evaluationMonth],
        set: {
          ...
          medRiskPct:            data.medRiskPct,
          lowRiskPct:            data.lowRiskPct,
          scoreHistogram:        data.scoreHistogram,
          topFeatureShapSummary: data.topFeatureShapSummary,
  ...
  async getModelEvaluationRuns(modelId: number): Promise<ModelEvaluationRun[]> {
    return db.select().from(modelEvaluationRuns)
      .where(eq(modelEvaluationRuns.modelId, modelId))
      .orderBy(modelEvaluationRuns.evaluationMonth);
  }
