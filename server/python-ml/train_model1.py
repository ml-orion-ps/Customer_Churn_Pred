...
def classify_risk_band_from_rank(rank_index, total_count):
    """Assign notebook-style risk bands from descending churn-probability rank."""
    if total_count <= 0:
        return 'Low Risk'

    rank_pct = float(rank_index + 1) / float(total_count)
    if rank_pct <= 0.05:
        return 'Very High Risk'
    if rank_pct <= 0.15:
        return 'High Risk'
...
class ChurnPreprocessedEstimator(BaseEstimator, ClassifierMixin):
    ...
    def _build_model(self):
        ...
        elif self.model_family == "decision_tree": # Add this block
...
def train_single_model(algorithm, X, y, feature_cols, custom_params=None, df=None,
    ...
    algo_to_family = {
        ...
        'Support Vector Machine': 'support_vector_machine'  # ✅ ADDED
    }
...
def main():
    ...
    try:
        ...
        # Dynamic validation against our MODEL_FAMILY_DISPLAY_NAMES
        valid_display_names = set(MODEL_FAMILY_DISPLAY_NAMES.values()) | {"Auto"}
        if algorithm not in valid_display_names:
            raise ValueError(
                f"Algorithm '{algorithm}' is not supported. "
                f"Supported algorithms: {', '.join(sorted(valid_display_names))}"
            )
